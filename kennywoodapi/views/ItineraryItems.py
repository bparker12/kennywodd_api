...
class ItineraryItemSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        model = Itinerary
        # has to go and look at URLs
        url = serializers.HyperlinkedIdentityField(
            view_name='itineraryitem',
            lookup_field='id'
        )
        # this is what you want back
        fields = ('id', 'url', 'starttime', 'attraction')
        # depth dictates how far you can expand out from the database. this will stop at 1 layer down if it is 1
        depth = 2


class ItineraryItems(ViewSet):
    """Itenerary items for Kennywood Customers"""
